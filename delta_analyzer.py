#!/usr/bin/env python3
"""
DELTA ANALYZER v10 — Chainlink uniquement + fallback CLOB robuste
=================================================================================
Sources :
  RTDS WebSocket  wss://ws-live-data.polymarket.com
    → crypto_prices_chainlink : prix Chainlink (7 assets)

  Market WebSocket wss://ws-subscriptions-clob.polymarket.com/ws/market
    → new_market      : detection instantanee des nouveaux marches
    → market_resolved : resolution instantanee
    → last_trade_price: prix du dernier trade
    → best_bid_ask    : prix du carnet en temps reel

  Gamma API https://gamma-api.polymarket.com/markets
    → recuperation des token IDs via slug (une seule fois par marche)

Dashboard : http://0.0.0.0:8080
DB        : /root/delta_analysis.db

Changements v10 :
  - Suppression complete Binance (PRIX_BN, delta_bn, direction_bn, concordant, signal_force)
  - Signal unique : delta_cl (Chainlink) — source de resolution Polymarket
  - Fallback REST CLOB appele token par token si l'un des deux prix est manquant
  - Dashboard et analyses epures : uniquement metriques CL
"""
...
sess.headers.update({'User-Agent': 'DeltaAnalyzer/10.0'})
...
# Marches actifs
# {market_key: {slug, condition_id, asset, duree, start_ts, end_ts, idu, idd}}
MARCHES: dict = {}
...
_HTML = "<html><body style='background:#080b0f;color:#00d4ff;font-family:monospace;padding:40px'><h2>⚡ Delta Analyzer v10</h2><p>Demarrage...</p></body></html>"
...
def scanner(db):
    ...
    for mkey, minfo in marches_copy.items():
        ...
        spread  = round(abs(pu - (1 - pd)), 4) if pu and pd else None
        ...
        for t in TIMINGS:
            ...
            with _lock_db:
                ...
                db.execute("""INSERT INTO snaps
                    (market_key,asset,duree,timing,ts,
                     ...
                     direction_cl,
                     clob_up,clob_dn,spread,prix_entree)
                    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                    (mkey,asset,dl,t,now,
            ...
            print(f"[SNAP] {asset.upper()} {dl} T-{t}s | "
                  ...
                  f"dir={dir_cl or 'N/A'} "
                  f"CLOB_UP={f'{pu:.4f}' if pu else 'N/A'} "
                  f"CLOB_DN={f'{pd:.4f}' if pd else 'N/A'}")
...
def enrich_tokens(cid: str) -> tuple:
    try:
...
# ══════════════════════════════════════════════════════
# ANALYSE STATISTIQUE — Chainlink uniquement
# ══════════════════════════════════════════════════════
...
# Zones CLOB — prix du cote gagnant (clob_up si UP, clob_dn si DOWN)
CLOB_ZONES = [
    ('0.70-0.75', 0.70, 0.75),
    ('0.75-0.80', 0.75, 0.80),
    ('0.80-0.85', 0.80, 0.85),
...
def analyser(db) -> dict:
    ...
    # ── Analyse principale : asset × delta × timing × zone CLOB ──
    # Structure : analyse[asset][delta_label][timing] = {zone_label: {wr, n}}
    analyse = {}
    for asset in ASSETS:
        analyse[asset] = {}
        for dlabel, dmin, dmax in DELTA_TRANCHES:
            analyse[asset][dlabel] = {}
            for t in TIMINGS:
                analyse[asset][dlabel][t] = {}
                for zlabel, zmin, zmax in CLOB_ZONES:
                    ...
                    """, (asset, t, dmin, dmax, zmin, zmax, zmin, zmax)).fetchone()
                    n, w = row
                    if n and n >= 3:
                        analyse[asset][dlabel][t][zlabel] = {
                            'n':  n,
...
def wc(w):
    ...
    if w >= 90: return '#44ff88'
    if w >= 85: return '#88ff44'
...
def make_html(R: dict) -> str:
    ...
    notice = f'<div class="notice">⏳ <strong>{res}</strong> cycles resolus — minimum 50 recommande</div>' if res < 50 else ''

    # Zones CLOB labels (colonnes du tableau)
    zone_labels = [z[0] for z in CLOB_ZONES]

    def cell(data):
        """Cellule WR coloree ou vide."""
        if not data:
            return '<td class="emp">—</td>'
        w = data['wr']
        n = data['n']
        return f'<td style="background:{wc(w)}18;color:{wc(w)};font-weight:700" title="N={n}">{w}%<span class="nn"> {n}</span></td>'

    # Generer les sections par asset
    sections = ''
    for asset in ASSETS:
        ...
        for dlabel, dmin, dmax in DELTA_TRANCHES:
            tdata = analyse.get(asset, {}).get(dlabel, {})

            # En-tete tableau : zones CLOB
            thead = '<tr><th>Timing</th>' + ''.join(f'<th>{z}</th>' for z in zone_labels) + '</tr>'
            ...
            table = f'<div class="tscroll"><table>{thead}{rows}</table></div>' if has_data else '<div class="emp-blk">En collecte...</div>'
            asset_html += f'''
            <div class="dtab">
...
.asset-hdr{display:flex;align-items:baseline;gap:12px;margin-bottom:14px;padding-bottom:10px;border-bottom:1px solid var(--bd)}
.asset-name{font-size:18px;font-weight:800;color:var(--ac)}
.asset-prix{font-family:'JetBrains Mono',monospace;font-size:13px;color:var(--tx)}
.dtab{margin-bottom:14px}
.dlabel{font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:1.5px;color:var(--di);margin-bottom:6px;padding:3px 8px;background:var(--bg3);border-radius:4px;display:inline-block}
.tscroll{overflow-x:auto}
table{border-collapse:collapse;font-size:11px;min-width:500px}
th{text-align:center;padding:5px 8px;font-size:9px;text-transform:uppercase;letter-spacing:.8px;color:var(--di);border-bottom:1px solid var(--bd);white-space:nowrap}
th:first-child{text-align:left}
td{padding:5px 8px;border-bottom:1px solid rgba(28,34,48,.4);text-align:center;font-family:'JetBrains Mono',monospace;font-size:11px}
td:first-child{text-align:left}
td.tm{color:var(--tx);font-weight:700}
td.emp{color:var(--di)}
tr:hover td{background:var(--bg3)}
.nn{font-size:9px;opacity:.6;font-weight:400}
.emp-blk{color:var(--di);font-size:12px;padding:8px 0}
"""
...
<title>⚡ Delta Analyzer v10</title>
<style>{CSS}</style>
</head><body>
<div class="hdr">
  <div><div class="ttl">⚡ DELTA ANALYZER <span class="tag">v10</span></div>
  <div class="sub">Polymarket • Chainlink RTDS • WR par asset / delta / timing / zone CLOB</div></div>
...
<div class="kpis">
  ...
  <div class="kp"><div class="kl">Resolus</div><div class="kv">{res}</div></div>
  <div class="kp"><div class="kl">Depuis</div><div class="kv" style="font-size:12px">{st.get('debut','—')}</div></div>
</div>
{notice}
{sections}
</body></html>"""
...
def main():
    global _HTML
    print("="*60)
    print("   ⚡  DELTA ANALYZER v10 — Chainlink uniquement")
    print("="*60)
    print(f"   Assets    : {', '.join(a.upper() for a in ASSETS)}")
    print(f"   Durees    : {', '.join(d[0] for d in DUREES)}")
    print(f"   Signal    : Chainlink uniquement (Binance supprime)")
