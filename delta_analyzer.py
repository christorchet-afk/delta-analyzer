#!/usr/bin/env python3
"""
DELTA ANALYZER v13 — Liquidite CLOB par zone de prix precise
=================================================================================
Changements v13 :
  - Liquidite capturee en 2 zones separees :
    → liq_96_99 : volume $ asks entre 0.96 et 0.999 (zone principale bot)
    → liq_93_96 : volume $ asks entre 0.93 et 0.96 (zone potentielle)
  - Nombre de niveaux par zone : liq_96_lvls, liq_93_lvls
  - Dashboard : affichage des 2 zones par asset/duree
  - Capture toujours a T-10s, T-5s, T-3s uniquement
"""
...
sess.headers.update({'User-Agent': 'DeltaAnalyzer/13.0'})
...
_HTML = "<html><body style='background:#080b0f;color:#00d4ff;font-family:monospace;padding:40px'><h2>⚡ Delta Analyzer v13</h2><p>Demarrage...</p></body></html>"
...
def scanner(db):
    ...
    for mkey, minfo in marches_copy.items():
        ...
        for t in TIMINGS:
            ...
            liq_96_99 = liq_96_lvls = liq_93_96 = liq_93_lvls = None
            if t in (10, 5, 3):
                tid_dir = idu if dir_cl == 'UP' else idd
                if tid_dir:
                    liq_96_99, liq_96_lvls, liq_93_96, liq_93_lvls = get_book_liquidite(tid_dir)
            ...
            with _lock_db:
                db.execute("""INSERT INTO snaps
                    (market_key,asset,duree,timing,ts,
                     ...
                     direction_cl,
                     clob_up,clob_dn,
                     liq_96_99,liq_96_lvls,liq_93_96,liq_93_lvls,
                     prix_entree)
                    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                    (mkey,asset,dl,t,now,
                ...
                db.commit()
            liq_str = ''
            if liq_96_99: liq_str = f'LIQ96={liq_96_99:.1f}$({liq_96_lvls}niv)'
            if liq_93_96: liq_str += f' LIQ93={liq_93_96:.1f}$'
...
def get_book_liquidite(token_id: str) -> tuple:
    ...
    Retourne (liq_96_99, liq_96_lvls, liq_93_96, liq_93_lvls)

    Zone principale  : 0.96 → 0.999 (zone d'entrée réelle du bot)
    Zone potentielle : 0.93 → 0.96  (si on elargit les critères)

    Timeout 2s pour ne pas bloquer le scanner.
    """
    try:
        ...
        return (
            round(liq_96_99, 2) if liq_96_99 > 0 else None,
            lvls_96 if lvls_96 > 0 else None,
            round(liq_93_96, 2) if liq_93_96 > 0 else None,
            lvls_93 if lvls_93 > 0 else None,
        )
    except Exception:
        return None, None, None, None
    try:
...
def analyser(db) -> dict:
    ...
    # ── Liquidite par asset/duree/timing — 2 zones ──
    liquidite = {}
    for asset in ASSETS:
        liquidite[asset] = {}
        for dl, _ in DUREES:
            rows = db.execute("""
                SELECT timing,
                  AVG(liq_96_99)  as vol_96_moy,
                  AVG(liq_93_96)  as vol_93_moy,
                  AVG(liq_96_lvls) as lvls_96_moy,
                  COUNT(*) as n
                FROM snaps
                WHERE asset=? AND duree=?
                  AND timing IN (10,5,3)
                  AND liq_96_99 IS NOT NULL
            ...
            if rows:
                liquidite[asset][dl] = {
                    r[0]: {
                        'vol_96':  round(r[1] or 0, 1),
                        'vol_93':  round(r[2] or 0, 1),
                        'lvls_96': round(r[3] or 0, 1),
                        'n':       r[4],
                    } for r in rows
                }
        ...
        for dl, _ in DUREES:
            ...
            if ldata:
                ...
                if ref:
                    v96 = ref['vol_96']
                    v93 = ref['vol_93']
                    lc96 = '#00ff88' if v96 >= 100 else '#ffcc00' if v96 >= 20 else '#ff4444'
                    lc93 = '#88aaff' if v93 >= 50 else '#556688'
                    liq_info += (
                        f'<span class="liq-badge" style="border-color:{lc96};color:{lc96}">'
                        f'{dl} 0.96-0.99: ~{v96:.0f}$</span>'
                    )
                    if v93 > 0:
                        liq_info += (
                            f'<span class="liq-badge" style="border-color:{lc93};color:{lc93}">'
                            f'{dl} 0.93-0.96: ~{v93:.0f}$</span>'
                        )
...
<title>⚡ Delta Analyzer v13</title>
<style>{CSS}</style>
</head><body>
<div class="hdr">
  <div><div class="ttl">⚡ DELTA ANALYZER <span class="tag">v13</span></div>
  <div class="sub">Polymarket • Chainlink RTDS • Liquidite par zone 0.96-0.99 / 0.93-0.96</div></div>
...
def main():
    global _HTML
    print("="*60)
    print("   ⚡  DELTA ANALYZER v13 — Liquidite par zone")
    print("="*60)
    print(f"   Assets    : {', '.join(a.upper() for a in ASSETS)}")
    print(f"   Durees    : {', '.join(d[0] for d in DUREES)}")
    print(f"   Signal    : CL + momentum + liquidite 0.96-0.99 / 0.93-0.96")
