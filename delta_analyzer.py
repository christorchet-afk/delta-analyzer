#!/usr/bin/env python3
"""
DELTA ANALYZER v11 — Analyse optimisee pour configuration UltimateShoot
=================================================================================
Sources :
...
sess.headers.update({'User-Agent': 'DeltaAnalyzer/11.0'})
...
_HTML = "<html><body style='background:#080b0f;color:#00d4ff;font-family:monospace;padding:40px'><h2>⚡ Delta Analyzer v11</h2><p>Demarrage...</p></body></html>"
    ...
    for mkey, minfo in marches_copy.items():
        ...
        for t in TIMINGS:
            ...
            print(f"[SNAP] {asset.upper()} {dl} T-{t}s | "
                  ...
                  f"SPR={f'{spread:.4f}' if spread else 'N/A'}")
...
def analyser(db) -> dict:
    ...
    # ── Stats spread par asset/duree (qualite du marche) ──
    spreads = {}
    for asset in ASSETS:
        spreads[asset] = {}
        for dl, _ in DUREES:
            row = db.execute("""
                ...
                FROM snaps
                WHERE resultat IS NOT NULL AND asset=? AND duree=?
                  AND spread IS NOT NULL AND timing=5
            """, (asset, dl)).fetchone()
            avg_spr, n = row
            if n and n >= 5:
                spreads[asset][dl] = round(avg_spr or 0, 4)
    R['spreads'] = spreads
...
def make_html(R: dict) -> str:
    ...
    for asset in ASSETS:
        ...
        # Spread info par duree
        spr_info = ''
        for dl, _ in DUREES:
            spr = spreads.get(asset, {}).get(dl)
            if spr is not None:
                spr_color = '#00ff88' if spr < 0.01 else '#ffcc00' if spr < 0.03 else '#ff4444'
                spr_info += f'<span class="spr-badge" style="border-color:{spr_color};color:{spr_color}">{dl} spread={spr:.4f}</span>'
        ...
        <div class="asset-card">
          <div class="asset-hdr">
            ...
            <span class="spr-wrap">{spr_info}</span>
          </div>
          {asset_html}
        </div>'''
...
.spr-wrap{display:flex;gap:6px;flex-wrap:wrap;margin-left:auto}
.spr-badge{font-family:'JetBrains Mono',monospace;font-size:9px;padding:2px 6px;border-radius:3px;border:1px solid;opacity:.8}
...
<title>⚡ Delta Analyzer v11</title>
<style>{CSS}</style>
</head><body>
<div class="hdr">
  <div><div class="ttl">⚡ DELTA ANALYZER <span class="tag">v11</span></div>
  <div class="sub">Polymarket • Chainlink RTDS • WR+PNL par asset / duree / delta / timing / zone CLOB</div></div>
...
def main():
    global _HTML
    print("="*60)
    print("   ⚡  DELTA ANALYZER v11 — Analyse optimisee")
    print("="*60)
    print(f"   Assets    : {', '.join(a.upper() for a in ASSETS)}")
    print(f"   Durees    : {', '.join(d[0] for d in DUREES)}")
    print(f"   Signal    : Chainlink + momentum court terme")
