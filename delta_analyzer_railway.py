...
def on_message(ws, message):
    try:
        ...
        sym = d.get('s', '').lower().replace('usdt', '')
...
def on_close(ws, *args):
    print("  [WS] Fermé — reconnexion dans 5s...")
    time.sleep(5)
    start_ws()

def on_open(ws):
    connected = list(PRIX_LIVE.keys())
    print(f"  [WS] Connecté ✅ ({len(connected)} assets)")

def start_ws():
    streams = "/".join([f"{a}usdt@ticker" for a in ASSETS])
